...
def svm_model():
    pd_data = pd.read_csv('DetectPD-ADASYN.csv')
    ...
    for x in range(2500):
        ...
        print("count: ", x + 1, "accuracy: ", accuracy)
        if x == 0:
            lowModel = clf
            highModel = clf
...
t1 = threading.Thread(target=svm_model)
t2 = threading.Thread(target=svm_model)
t3 = threading.Thread(target=svm_model)
t4 = threading.Thread(target=svm_model)
